...
function move_player(dt, bed)
	...
	for i, bed_part in ipairs(bed) do
		...
		if touched_bedpart and not bed_part.found then
			bed_part.found=true
			if bed_part.behind then table.insert(level_0["under_bed"], bed_part)
			else table.insert(level_0["over_bed"], bed_part) end
			player.collect:play()
		end
	end
end
...
function check_detection(a, b)
	return a.x + a.width > b.x - b.detection
	and a.x < b.x + b.width + b.detection
    and a.y + a.height > b.y - b.detection
    and a.y < b.y + b.height + b.detection
end
